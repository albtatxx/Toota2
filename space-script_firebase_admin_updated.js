...
// =====================
// تم إلغاء "اسم المستخدم" نهائياً من الموقع
// =====================

const ANON_USER_NAME = 'مجهول';

const USER_FIRST_NAME_KEY = 'spacePlatform_firstName_v1';
const FIRESTORE_ATTEMPTS_COLLECTION = 'examAttempts_v1';

function getSavedFirstName() {
    return String(localStorage.getItem(USER_FIRST_NAME_KEY) || '').trim();
}
...
function setSavedFirstName(firstName) {
    ...
    localStorage.setItem(USER_FIRST_NAME_KEY, clean);

    ensureCurrentUserObject();
    appData.currentUser.name = clean;

    try {
        localStorage.setItem('spacePlatform_userData', JSON.stringify(appData.currentUser));
    } catch (e) {
        // تجاهل
    }
...
function loadUserData() {
    const savedData = localStorage.getItem('spacePlatform_userData');
